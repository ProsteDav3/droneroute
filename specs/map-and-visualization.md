...
- **The 3D let view matches the mission's own camera field of view**, not the map's own default one — Orbit's whole-object framing (see [Templates](templates.md)) computes gimbal angles assuming the mission's actual selected drone's lens (or a typical wide-angle stand-in), and the flythrough now widens the map's rendering field of view to match while it's active, restoring the map's normal view afterward. Without this, the framing math's angle was correct but rendered through a visibly narrower lens than it assumed — cropping exactly the margin the framing was counting on.
