...
- **Live fleet status**: when DJI Cloud is configured, a "DJI Cloud — zařízení" panel in the sidebar lists devices bound to the workspace (online/offline) — the aircraft with a plane icon, its remote controller or dock with a distinct gamepad/box icon, so the two don't read as separate aircraft — and recent Health Management System (HMS) warnings reported by whichever device is currently online, so a pilot can catch a gimbal fault or similar before a flight without opening Pilot 2's own menus. A warning only shows while its device is online, and shows how long ago it was reported (e.g. "před 3 h"), since the platform keeps every warning it has ever seen with no "resolved" marker — a code that fired once during an earlier self-test would otherwise look exactly like an active problem right now.
- **Live telemetry HUD**: clicking a bound aircraft (or automatically, when only one is bound) shows a real-time readout of signal strength, the aircraft's own battery and its paired remote controller's battery separately, altitude above sea level (ASL) and above the takeoff point (ALT), distance from the home point, climb rate, flight speed, and wind speed — mirroring the fields DJI Pilot's own flight screen shows, updating live over the same connection as the map marker. If no fresh update has arrived for more than 15 seconds (the connection likely dropped silently), the readout dims and warns that the values shown may no longer match the aircraft's actual current state.
