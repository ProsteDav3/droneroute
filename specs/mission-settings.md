...
- **Clicking "Nová trasa" (new mission) asks which drone you're flying** before opening the editor, pre-selected to your account's default drone. This choice is saved with that mission specifically — changing your account's default drone later doesn't retroactively update missions you already created, so if you fly more than one drone model, this is where each mission actually gets the right one (it also determines the camera field of view used for Orbit's whole-object framing — see [Templates](templates.md)). Cancelling leaves you on the mission list without creating anything.
