...
- Select two adjacent waypoints and click "Interpolovat" (with a count, 1-50) to insert that many new points evenly spaced between them, with position, height, speed, and gimbal pitch linearly interpolated from one to the other — quicker than manually placing and adjusting each point when a segment needs finer coverage.
...
- Lock a range of waypoints by typing it: the row above the waypoint list takes a from/to pair ("Body 1 – 40") and locks or unlocks exactly those, without hunting for both ends in a list of seventy. The numbers are the ones shown on the map markers.
- Lock a waypoint in place with the padlock icon in its sidebar row, or lock a whole selection at once with "Zamknout" in the bulk toolbar. A locked waypoint doesn't move: it can't be dragged on the map, it sits out "Posun" and "Obrátit"-style whole-mission moves and rotations, it survives re-applying the template that created it, and it stays put when a [building](buildings.md) is reshaped. Built for long-running time-lapse contracts, where footage from earlier visits only lines up if the points already filmed stay exactly where they were. You can still edit a locked waypoint's settings by hand and still delete it — the lock guards against automatic and bulk movement, not against you.
...
- **Estimated photo data size**: the footer's photo-count badge also shows an estimated total JPEG data size (e.g. "24 (~180 MB)") for the mission's selected camera, based on typical DJI compression at that camera's resolution — a planning estimate for storage/transfer, not a guaranteed figure. Shown only when the selected camera's photo resolution is known.
