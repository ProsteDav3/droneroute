...
- Move a whole wall at once: drag the pale-blue handle in the middle of any edge and both its corners travel together, keeping the wall parallel to where it was. A plain click on the same handle still inserts a new corner there. Corner handles (the white ones) still move a single corner, and a right-click on one removes it.
- Reshape a building mid-contract and have the route follow it: after you change a footprint or its height, a bar appears offering to move the waypoints to match the new recommended standoff, showing the old and new radius and how many points would move. The route only changes when you click **Použít**; **Zrušit** keeps the new footprint and leaves the flight untouched. Locked waypoints (see [mission planning](mission-planning.md)) never move, so a time-lapse can grow with the building without disturbing the shots already filmed. While the bar is open, the map shows dashed leaders from each affected waypoint to where it would land.
...
- Where a locked stretch meets the waypoints that are free to move, the reflow ramps the move in gradually instead of stepping straight to full size: the first waypoint after the locked run barely moves, and the move grows to its full size over the next few. The "Přechod" field in the reflow bar sets how many waypoints that ramp spans (8 by default, 0 for no ramp), and the map preview updates as you change it. Without it the flight path takes a visible kink exactly where the already-filmed half ends, and the cut between two visits jumps.
- The reflow after a building edit keeps each waypoint's own bearing around the building and its own margin beyond the recommended radius, so the shape and order of the route survive — it re-hangs the existing points at the new distance rather than regenerating the orbit from scratch. Altitude, speed and gimbal pitch are left alone, so the moved points keep matching the look of the locked ones.
