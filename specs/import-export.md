...
- **Live telemetry HUD**: clicking a bound aircraft (or automatically, when only one is bound) shows a real-time readout of signal strength, battery, altitude above sea level (ASL) and above the takeoff point (ALT), distance from the home point, climb rate, flight speed, and wind speed — mirroring the fields DJI Pilot's own flight screen shows, updating live over the same connection as the map marker.
