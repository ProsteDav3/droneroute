...
- **Lock the POI separately from the orbit's center**: by default, the camera aims at the flight circle's own center. Check "Uzamknout POI" ("Lock POI") to drop an independent, fixed camera target at the current center — a second draggable marker appears for it. With it locked, you can then move or resize the flight circle (drag the center handle, or change the radius) to fly farther from or closer to the subject without moving what the camera is looking at; gimbal pitch is then recalculated per waypoint since distance to the fixed target varies around the circle.
...
3. For Orbit, Grid, and Facade: click-and-drag on the map to place the template. For Solar: click to place each boundary point, click near the first point or double-click to close the shape, then click two more points along a panel row to set the flight direction. For Pencil: click-and-drag to draw a freehand path.
