...
/**
 * Waypoints that track a camera target but carry no gimbal command at all.
 *
 * Aiming a DJI mission takes two separate things: the heading mode
 * (`towardPOI`, which turns the aircraft) and actual gimbal commands
 * (`gimbalRotate` to set the opening angle, `gimbalEvenlyRotate` to walk it
 * along each leg). A route with the first and none of the second flies with
 * the camera pointed at the target horizontally while the gimbal holds
 * whatever angle it happened to have — field-verified on a Matrice 4T, and
 * the reason a whole KCP mission plus 71 segments had to be re-planned and
 * re-uploaded.
 *
 * Templates have emitted the gimbal commands since then, so a mission
 * missing them was generated by an older build still cached in the browser.
 * Warning, never a block: a hand-built route may legitimately track a target
 * and leave the gimbal to the pilot.
 */
export function staleAimingWarning(
  waypoints: { headingMode?: string; actions?: { actionType: string }[] }[],
): string | null {
  const tracking = waypoints.filter((wp) => wp.headingMode === "towardPOI");
  if (tracking.length === 0) return null;
  const hasGimbalCommand = waypoints.some((wp) =>
    (wp.actions ?? []).some(
      (a) =>
        a.actionType === "gimbalRotate" ||
        a.actionType === "gimbalEvenlyRotate",
    ),
  );
  if (hasGimbalCommand) return null;
  return `${tracking.length} bodů trasy míří na cíl kamery, ale trasa neobsahuje žádný povel gimbalu — dron se otočí správně, ale kamera zůstane v původním náklonu a nezaostří. Obnovte stránku (Ctrl+Shift+R) a šablonu znovu použijte, pak nahrajte znovu.`;
}
