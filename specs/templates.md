...
- **Move the template config panel out of the way**: drag it by its title bar (the grip icon next to the template name) to reposition it anywhere over the map, instead of having to zoom out to see what's underneath it. It stays within the map area and resets to its default position the next time a template panel opens.
