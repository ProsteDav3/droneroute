...
- **Export segments**: split a multi-waypoint route into consecutive one-leg
  ...
  for that leg, a one-second settle, then a focus — before its recording
  starts, rather than relying on the opening angle and focus that the parent
  route only performs once at its very beginning. Routes that don't aim at a
...
- **Live fleet status**: when DJI Cloud is configured, a "DJI Cloud — zařízení" panel in the sidebar lists devices bound to the workspace (online/offline) — the aircraft with a plane icon, its remote controller or dock with a distinct gamepad/box icon, so the two don't read as separate aircraft — and recent Health Management System (HMS) warnings reported by whichever device is currently online, so a pilot can catch a gimbal fault or similar before a flight without opening Pilot 2's own menus. A device with no platform-reported name (common for a tablet/phone running the Pilot 2 app itself as the gateway, rather than a dedicated RC unit) shows as "DJI Pilot 2" instead of its bare serial number. A warning only shows while its device is online AND was reported since that device's current connection began, and shows how long ago it was reported (e.g. "před 3 h") — the platform keeps every warning it has ever seen with no "resolved" marker, so without both checks a code that fired once during an earlier session (or an earlier self-test) would look exactly like an active problem right now.
- **Live telemetry HUD**: clicking a bound aircraft (or automatically, when only one is bound) shows a real-time readout of signal strength, the aircraft's own battery and its paired remote controller's battery separately, altitude above sea level (ASL) and above the takeoff point (ALT), distance from the home point, climb rate, flight speed, and wind speed — mirroring the fields DJI Pilot's own flight screen shows, updating live over the same connection as the map marker. The aircraft's battery reading is its own physical battery's reported percentage (or the average across all batteries, for a dual-battery aircraft) rather than the platform's separate aggregate estimate, which can read several points lower than what's actually on the battery. If no fresh update has arrived for more than 15 seconds (the connection likely dropped silently), the readout dims and warns that the values shown may no longer match the aircraft's actual current state.
