...
- **Live fleet status**: when DJI Cloud is configured, a "DJI Cloud — zařízení" panel in the sidebar lists devices bound to the workspace (online/offline) and recent Health Management System (HMS) warnings reported by the aircraft — so a pilot can catch a gimbal fault or similar before a flight without opening Pilot 2's own menus. Each warning shows how long ago it was reported (e.g. "před 3 h"), since the platform keeps every warning it has ever seen with no "resolved" marker — a code that fired once during an earlier self-test would otherwise look exactly like an active problem right now.
