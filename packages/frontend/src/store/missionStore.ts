...
// Undo/redo controls and the autosaved-draft-to-localStorage mechanism live
// in missionDraft.ts (import `useMissionHistory`/`peekMissionDraft`/
// `clearMissionDraft`/`restoreMissionDraft` from there) — both only ever
// read/write this store from the outside (`.temporal`, `.getState()`,
// `.subscribe()`), never from inside the state creator below, so they don't
// need to live in this file. Not re-exported here: doing so would make this
// module and missionDraft.ts import each other, and missionDraft.ts's
// top-level `useMissionStore.subscribe(...)` call would then run before
// this file's own `create()` call has executed.
