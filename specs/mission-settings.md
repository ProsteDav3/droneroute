...
- **You're asked which drone you're flying** before that choice matters — either clicking "Nová trasa" (new mission), or picking a template (Orbit, Grid, ...) directly on a mission that's still completely empty and unsaved (drawing straight onto a building without going through "Nová trasa" first counts too). Pre-selected to your account's default drone. This choice is saved with that mission specifically — changing your account's default drone later doesn't retroactively update missions you already created, so if you fly more than one drone model, this is where each mission actually gets the right one (it also determines the camera field of view used for Orbit's whole-object framing — see [Templates](templates.md)). Cancelling leaves the mission untouched, with nothing added. Once a mission has any content, applying more templates to it no longer asks again — the drone was already settled the first time.
